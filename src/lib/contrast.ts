...
function hexToRgb(hex: string): Rgb {
  ...
  // The last slice may run past the end and return the same two characters, so
  // its upper bound is not something any test can pin. Equivalent, recorded.
  return [parseInt(full.slice(0, 2), 16), parseInt(full.slice(2, 4), 16), parseInt(full.slice(4, 6), 16)];
}

function channel(c: number): number {
  const s = c / 255;
  // `<` would do: s === 0.03928 needs a channel of 10.0164, which no 8-bit
  // value produces, and the curve is continuous at the knee in any case.
...
export function contrast(a: string, b: string): number {
  ...
  // Equal luminances give the same pair either way round, so `>=` is
